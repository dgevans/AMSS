...
class PolicyMap(object):
    ...
    def policyResiduals(self,state,z):
        ...
        T  = ((xi-pi1)/(alpha*pi2))**(-1./Para.sigma)
        
        #Now do constraints
        res = zeros(4*S+1)
        res[:S] = x*p*Uc/( beta*EpUc ) - Uc*(c-T) - Ul*l - xprime
        res[S:2*S] = theta*l*pi1 - c*pi1 - g - pi2*T
...
def fitPolicyFunction(Para,PolicyF):
    ...
    for s_ in range(S):
        ...
        for s in range(S):
            cf[s_,s] = interp1d(mugrid,Policies[:,s],k)#Spline(mugrid,Policies[:,s],[2])
            lf[s_,s] = interp1d(mugrid,Policies[:,s+S],k)#Spline(mugrid,Policies[:,s+S],[2])
            muprimef[s_,s] = interp1d(mugrid,Policies[:,s+2*S],k)#Spline(mugrid,Policies[:,s+2*S],[2])
            xif[s_,s] = interp1d(mugrid,Policies[:,s+3*S],k)#Spline(mugrid,Policies[:,s+3*S],[2])
        #xf.append(Spline(mugrid,Policies[:,4*S],[1]))
        xf.append(interp1d(mugrid,Policies[:,4*S],k))
